...
# RuntimeError: CUDA out of memory. Tried to allocate 48.00 MiB (GPU 0; 10.76 GiB total capacity; 8.08 GiB already allocated; 49.88 MiB free; 8.11 GiB reserved in total by PyTorch)

def compile():
    model, device = image_zoom.get_tvm_model()

    todos.data.mkdir("output")
    if not os.path.exists("output/image_zoom2x.so"):
        input = torch.randn(SO_B, SO_C, SO_H, SO_W)
        todos.tvmod.compile(model, device, input, "output/image_zoom2x.so")
...
def predict(input_files, output_dir):
    ...
    # load model
    tvm_model = todos.tvmod.load("output/image_zoom2x.so", "cuda")
